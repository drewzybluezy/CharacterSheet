...
public class CSVLoader {
    ...
    public static List<Race> initRaces(Context context) {
        ...
        for (int i = 0; i < csvList.size(); ++i) {
            ...
            for (int j = 0; j < CSheetReusables.abilityAbbrevs.length; j++) {
                checkMods(abilityMods, j, abilityMod1, abilityMod2, CSheetReusables.abilityAbbrevs[j]);
            }
            ...
            for (int j = 0; j < CSheetReusables.skillAbbrevs.length; j++) {
                checkMods(skillMods, j, skillMod1, skillMod2, skillMod3, CSheetReusables.skillAbbrevs[j]);
            }
    ...
    public static List<CharClass> initClasses(Context context) {
        ...
        for (int i = 0; i < csvList.size(); ++i) {
            ...
            currentClass.setHealingSurgesPerDay(Integer.parseInt(currentArray[10]));
    ...
    private static void setClassSkills(String toSplit, CharClass currentClass) {
        ...
        for (String s : toSplit.split("/")) {
            if (s.charAt(0) == '&') {
                ...
                availableSkillGroups.add(Integer.valueOf(Integer.parseInt(s.charAt(0) + "")));
                s = s.substring(2, s.length());
                for (String t : s.split(";")) {
                    for (int i = 0; i < CSheetReusables.skillAbbrevs.length; i++) {
                        if (t.equals(CSheetReusables.skillAbbrevs[i]) && availableSkills[i] != -1) {
    ...
    private static void checkMods(int[] toMod, int index, String arg0, String arg1, String compareTo) {
        if (arg0.equals(compareTo) || arg1.equals(compareTo)) toMod[index] += 2;
    }

    private static void checkMods(int[] toMod, int index, String arg0, String arg1, String arg2, String compareTo) {
        if (arg0.equals(compareTo) || arg1.equals(compareTo) || arg2.equals(compareTo)) toMod[index] += 2;
    }
    ...
    private static String buildRaceDescription(Race race) {
        ...
        String languageDisplay = getFormattedLanguage(languages[0]) + ", " + getFormattedLanguage(languages[1]);
        if (languageDisplay.lastIndexOf(',') == (languageDisplay.length() - 2)) {
            languageDisplay = languageDisplay.substring(0, languageDisplay.length() - 2);
        }
    ...
    private static String getFormattedLanguage(String toFormat) {
        for (int i = 0; i < CSheetReusables.languageAbbrevs.length; i++) {
            ...
            if (toFormat.equals("CHOICE")) return "Choice";
        }
        return "";
    }
}
